...
| `StoreItemInventory` | table | **Weekly input.** (ItemCode, StoreCode, OnHandQty, InTransitQty, MinQty, MaxQty) - resolves 2026-09-05, replaces `INV_SBS_QTY_V_EXT` (on-hand) and the hardcoded-0 in-transit placeholder. Keyed on the business Store Code. Populated by `scripts/import-store-inventory.js` (bulk insert - ~114k rows) from `assets/Stores_Qtys_and_MinMax.xlsx`. `MinQty`/`MaxQty` are stored but not currently used by the allocation calc. See `business-rules.md` for the floor-at-0 netting rule. |
