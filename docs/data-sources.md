...
| `INV_SBS_QTY_V_EXT` | view | On-hand qty by item/store (`QTY`), plus `MIN_QTY`/`MAX_QTY`/`TRANSFER_IN_QTY`/`TRANSFER_OUT_QTY`. **This is our current on-hand source - not yet confirmed as authoritative with Wesley.** Store 470 (Ecommerce) has 2 rows per item here (one live-qty row, one min/max-planning row with qty 0) - we `SUM(QTY)` grouped by item+store to avoid double counting/fan-out. |
